...
class _PrivacyScreenState extends ConsumerState<PrivacyScreen> {
  ...
  Future<String?> _getPrivacyPolicyContent() async {
    // First try to get from SharedPreferences
    final storedContent = getIt<SharedPrefService>().getString(StorageKeys.appPrivacy);
    ...
    try {
      ...
      if (versionData != null && versionData.data.privacyPolicy.isNotEmpty) {
        return _cleanHtmlContent(versionData.data.privacyPolicy);
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<String?>(
        future: _getPrivacyPolicyContent(),
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  Text('Failed to load privacy policy', style: theme.textTheme.titleMedium),
                  SizedBox(height: 8.h),
          ...
          if (htmlContent.isEmpty) {
            return Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Icon(Icons.description_outlined, size: 48.w, color: theme.colorScheme.onSurface.withValues(alpha: 0.5)),
                  SizedBox(height: 16.h),
                  Text('No privacy policy available', style: theme.textTheme.titleMedium),
                  SizedBox(height: 8.h),
                  Text('Privacy policy content is not available', style: theme.textTheme.bodyMedium),
                ],
              ),
            );
          }
