...
import '../../check_version/state_notifier/version_notifier.dart';
...
class _SplashScreenState extends ConsumerState<SplashScreen> with SingleTickerProviderStateMixin {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(
                        width: 22.w,
                        height: 22.w,
                        child: CircularProgressIndicator(strokeWidth: 2.2, valueColor: AlwaysStoppedAnimation(theme.primaryColor)),
                      ),
                    ],
                  ),
                ),
  ...
  Future<void> _onSplashComplete() async {
    try {
      // version check with timeout; don't block navigation
      await ref.read(versionNotifierProvider.notifier).checkVersion().timeout(const Duration(seconds: 5), onTimeout: () => Future.value());
    } catch (_) {}

    // Ask notification permission AFTER animation finished
    try {
      // Await user's choice; if permanently denied, service returns immediately
