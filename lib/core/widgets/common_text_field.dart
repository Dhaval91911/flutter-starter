...
class NoSpaceInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    formattedText = newValue.text.replaceAll(RegExp(r'\s+'), ' ');

    // If the text changed due to space normalization, update the selection
    if (formattedText != newValue.text) {
      final removedSpaces = newValue.text.length - formattedText.length;
      final offset = (newValue.selection.baseOffset - removedSpaces).clamp(0, formattedText.length);
...
class CommonTextField extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Widget field = TextFormField(
      ...
      decoration:
          ...
          InputDecoration(
            ...
            filled: filled,
            fillColor: fillColor ?? (theme.brightness == Brightness.dark ? Colors.black : Colors.white),
            border: isShowBorder
                ? border ??
                      OutlineInputBorder(
    ...
    return Container(alignment: Alignment.center, child: field);
  }
}
